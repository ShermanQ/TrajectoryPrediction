...
class Hdf5Dataset():
      ...
      def __init__(self,padding,images_path,hdf5_file,scene,t_obs,t_pred,normalize,use_images,data_type,use_neighbors,augmentation,augmentation_angles,centers,use_masks = False,reduce_batches = True,predict_offsets = 0,predict_smooth=0,smooth_suffix = ""):
            ...
            self.dset_name = scene
            self.dset_types = "{}_types".format(scene)
      ...
      def get_ids(self,ids):
            ...
            scenes = [self.scene for _ in ids]
      ...
      def __load_images(self):
            ...
            img = torch.FloatTensor(cv2.imread(self.images_path.format(self.scene)))
            img = img.permute(2,0,1)
            images[self.scene] = img
            return images
