...
############# tcn trajectory embedding ####################
        self.tcn_traj = TemporalConvNet(device, self.convnet_embedding, self.num_channels, kernel_size, dropout_tcn)
        # init network
        self.tcn_att = TemporalConvNet(device, self.convnet_embedding, self.num_channels, kernel_size, dropout_tcn)


        # project traj conv features to dmodel
        self.conv_enc_traj = nn.Linear(input_length*convnet_embedding,dmodel)

        # project att conv features to dmodel
        self.conv_enc_att = nn.Linear(input_length*convnet_embedding,dmodel)
...
############# Predictor #########################################
    ...
    def forward(self,x):
        ...
        y_att = y_att.permute(0,2,1) # [B*Nmax],Tobs,Nfeat
        y_att = y_att.view(B,Nmax,Tobs,y_att.size()[2]).contiguous() # B,Nmax,Tobs,Nfeat
        y_att = y_att.view(B,Nmax,-1) # B,Nmax,Tobs*Nfeat

        y_traj = y_traj.permute(0,2,1) # [B*Nmax],Tobs,Nfeat
        y_traj = y_traj.view(B,Nmax,Tobs,y_traj.size()[2]).contiguous() # B,Nmax,Tobs,Nfeat
        y_traj = y_traj.view(B,Nmax,-1) # B,Nmax,Tobs*Nfeat

        

        x_att = self.conv_enc_att(y_att) # B,Nmax,dmodel
        ...
        y = y.view(B,Nmax,t_pred,self.input_dim) #B,Nmax,Tpred,Nfea
