...
import json
import sys 
from helpers.helpers_training import get_colors
...
class Animation():
    def __init__(self,data_params,prepare_params,eval_params):
        ...
        self.reports_dir = self.data_params["reports_evaluation"] + "{}/scene_reports/".format(self.eval_params["report_name"])
        self.scene_samples = self.reports_dir + "{}_samples.json"
        self.gif_name = self.reports_dir + "{}_{}.gif"

        self.correspondences = json.load(open(self.data_params["pixel2meters_trajnet"]))
        self.correspondences_trajnet = json.load(open(self.data_params["pixel2meters_trajnet"]))
        self.correspondences_manual = json.load(open(self.data_params["pixel2meters_manual"]))
