...
def main():
    ...
    if data["multiple_scalers"]:
        ...
        scaler = scene_scaler.SceneScaler(data_params_path,center,prepare_training_params["normalize"])        
    else:
        print("-----------unique scaler")
        scaler = scene_scaler.SceneScalerMultiScene(data_params_path,center,scene_list,prepare_training_params["normalize"])
        
    for scene in scene_list:
        print(scene)
        scaler.min_max_scale(scene) # if not scale, centers the scene
    print("DOne!")
